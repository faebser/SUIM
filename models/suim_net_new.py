...
def Suim_Decoder_RSB(enc_inputs, n_classes):
    ...
    # match dims: previous code sliced spatial dims; replicate same behaviour with tf.slice inside Lambda
    dec_1 = Lambda(lambda x: x[:, :-2, :-2, :])(dec_1)
    dec_1 = ZeroPadding2D((1, 1))(dec_1)
    enc_2 = Lambda(lambda x: x[:, :-1, :-1, :])(enc_2)
    ...
    dec_2s = BatchNormalization()(dec_2s)
    dec_2s = UpSampling2D(size=2)(dec_2s)
    # padding for matching dimensions
    ...
    dec_3s = BatchNormalization()(dec_3s)
    # output layer
